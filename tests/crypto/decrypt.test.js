...
export default describe('decrypt', () => {
  ...
  const tests = [...ciphers]
    .map(([cipher, { key, nonce, ciphertext, additionalData }]) => [
      it(`should decrypt using ${cipher}`, async () => {
        expect(
          await decrypt(cipher, key, nonce, ciphertext, additionalData)
        ).toEqual(message)
      }),
  ...
  return tests.concat(
    it('should throw an exception for unsupported ciphers', async () => {
      const { key, nonce, ciphertext } = ciphers.get('AES-256-GCM')
      await expect(decrypt('hubba', key, nonce, ciphertext)).toReject(
        'Unsupported cipher'
      )
    })
  )
