...
import { hkdf } from '../../dist/crypto.js'

export default describe('hkdf', () => {
  ...
  const info = Uint8Array.from([225, 104, 38, 9, 70, 227, 194, 206])
  ...
  const tests = [...hashes].map(([hash, okm]) =>
    it(`should derive new a key using HKDF with ${hash}`, async () => {
      expect(await hkdf(hash, ikm, salt, info, okm.byteLength)).toEqual(okm)
    })
  )
  ...
  return tests.concat(
    it('should derive a key with the size of the given hash function when the last argument is omitted', async () => {
      const hash = 'SHA-512'
      const okm = hashes.get(hash)
      expect(await hkdf(hash, ikm, salt, info)).toEqual(okm.subarray(0, 64))
    }),

    it('should throw an exception when trying to use an unsupported hash algorithm', async () => {
      await expect(hkdf('hubba', ikm, salt, info)).toReject(
