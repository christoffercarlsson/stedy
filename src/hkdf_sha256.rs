use crate::{hmac_sha256, Error, HmacSha256};
...
fn expand(prk: &[u8; 32], info: Option<&[u8]>, okm: &mut [u8]) -> Result<(), Error> {
...
mod tests {
    ...
    fn test_hkdf_sha256() {
        ...
        hkdf_sha256(&ikm, Some(&salt), Some(&info), &mut okm).unwrap();
    ...
    fn test_hkdf_sha256_no_salt() {
        ...
        hkdf_sha256(&ikm, None, None, &mut okm).unwrap();

        let expected = [
            141, 164, 231, 117, 165, 99, 193, 143, 113, 95, 128, 42, 6, 60, 90, 49, 184, 161, 31,
            92, 94, 225, 135, 158, 195, 69, 78, 95, 60, 115, 141, 45, 157, 32, 19, 149, 250, 164,
            182, 26, 150, 200,
        ];
